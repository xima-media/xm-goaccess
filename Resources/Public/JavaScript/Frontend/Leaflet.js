function addressLeaflet() {
    ...
    obj.run = function () {
        obj.map = L.map('address__map').setView([51.505, -0.09], 13);
        ...
        var records = document.getElementById("address__records").children;
        for (var i = 0; i < records.length; i++) {
            var item = records[i];

            var marker = L.marker([item.getAttribute('data-lat'), item.getAttribute('data-lng')]).addTo(obj.map)
                .bindPopup(document.getElementById('address__record-' + item.getAttribute('data-id')).innerHTML);
